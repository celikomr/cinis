...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
    ...
    public static dynamic Create<T>(this MySqlConnection connection, T entity, MySqlTransaction? transaction = null)
    {
        ...
        var sql = $"insert into {GetTableSchema<T>()}{GetTableName<T>()} ({stringOfColumns}) values ({stringOfParameters}); select LAST_INSERT_ID();";
    ...
    public static dynamic Update<T>(this MySqlConnection connection, T entity, bool nullable = false, string? whereClause = null, MySqlTransaction? transaction = null)
    {
        if (connection is null)
        {
            throw new ArgumentNullException(nameof(connection));
