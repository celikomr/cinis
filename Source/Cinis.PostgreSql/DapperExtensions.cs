...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
    ...
    public static dynamic Create<T>(this NpgsqlConnection connection, T entity, NpgsqlTransaction? transaction = null)
    {
        ...
        var sql = $"insert into {GetTableSchema<T>()}{GetTableName<T>()} ({stringOfColumns}) values ({stringOfParameters}) returning {GetPrimaryKey<T>()?.GetCustomAttribute<ColumnAttribute>()?.Name}";

        var result = connection.ExecuteScalar(sql, entity, transaction);
        return result;
    }

    public static List<T> Read<T>(this NpgsqlConnection connection, dynamic? id = null, string? whereClause = null, NpgsqlTransaction? transaction = null)
    {
        if (connection is null)
        {
            throw new ArgumentNullException(nameof(connection));
...
#pragma warning restore CS8602 // Dereference of a possibly null reference.
    ...
    public static dynamic Delete<T>(this NpgsqlConnection connection, dynamic? id = null, string? whereClause = null, NpgsqlTransaction? transaction = null)
    {
        if (connection is null)
        {
            throw new ArgumentNullException(nameof(connection));
